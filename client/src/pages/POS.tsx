...
export default function POS() {
    ...
    if (isLoadingDevice || isLoadingRole || isLoadingProducts || isLoadingCashiers || pos.isLoadingSession) {
        return (
...
function SalesHistoryDialog({ open, onOpenChange, sales, isLoading, formatCurrency, onPrint }: any) {
    ...
    const handleVoidRequest = (sale: any) => {
    ...
    const handleVoidConfirm = async () => {
        if (!adminPassword) {
            setVoidError("Password Admin wajib diisi");
            return;
        }
        setVoidLoading(true);
        setVoidError("");
        ...
        try {
            ...
            const verifyRes = await fetch("/api/pos/verify-admin", {
                method: "POST",
            ...
            if (!verifyRes.ok || !verifyData.verified) {
                setVoidError(verifyData.message || "Password Admin salah");
                setVoidLoading(false);
                return;
            }

            // Step 2: Process void via RMA
            const saleRes = await fetch(`/api/sales/find/${voidTarget.uuid || voidTarget.id}`, { credentials: "include" });
            const saleData = await saleRes.json();

            if (!saleRes.ok) {
                setVoidError("Gagal mengambil detail penjualan");
                setVoidLoading(false);
                return;
            }
            ...
            const voidRes = await fetch("/api/rma/return", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
                body: JSON.stringify({
                    saleId: voidTarget.id,
                    returnNumber: `VOID-${voidTarget.id}`,
                    reason: "void_pos",
                    refundAmount: voidTarget.totalAmount,
                    refundMethod: "cash",
                    notes: `Void oleh ${verifyData.adminName} (${verifyData.role})`,
                    items
                })
            });
            ...
            if (!voidRes.ok) {
                ...
                setVoidError(err.message || "Gagal memproses void");
                setVoidLoading(false);
                return;
            }
            ...
            toast({
                ...
                description: `#${voidTarget.id} telah dibatalkan dan diotorisasi oleh ${verifyData.adminName}.`
            });
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { if (!v) setVoidTarget(null); onOpenChange(v); }}>
            <DialogContent className="bg-white text-slate-900 border-slate-200 rounded-[2rem] max-w-2xl max-h-[85vh] flex flex-col p-0 overflow-hidden shadow-3xl">
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-200">
                            ...
                            {sales.map((sale: any) => {
                                ...
                                return (
                                    <div key={sale.id} className={cn(
                                        "rounded-2xl p-4 flex items-center justify-between group transition-all border-l-4",
                                        isVoided
                                            ? "bg-red-50/50 border-l-red-400 opacity-60"
                                            : "bg-slate-50 border-l-blue-600/30 hover:bg-white/10"
                                    )}>
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] font-mono font-bold text-slate-900/20 bg-slate-50 px-1.5 py-0.5 rounded">#{sale.id}</span>
                                                <span className={cn("text-sm font-bold tracking-tight", isVoided ? "line-through text-red-400" : "text-slate-900")}>
                                                    {formatCurrency(sale.totalAmount)}
                                                </span>
                                                <Badge variant="outline" className={cn("text-[9px] py-0 h-4 uppercase font-black", isVoided ? "border-red-300 text-red-500 bg-red-50" : "border-slate-200 text-slate-600")}>
                                                    {isVoided ? "VOID" : sale.paymentMethod}
                                                </Badge>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Button
                                                size="icon"
                                                variant="ghost"
                                                className="h-10 w-10 text-slate-500 hover:text-blue-600 hover:bg-blue-600/10 rounded-xl"
                                            ...
                                            {!isVoided && (
                                                <Button
                                                    size="sm"
                                                    variant="ghost"
                                                    className="h-10 px-3 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl text-[10px] font-black uppercase tracking-wider gap-1.5"
                                                    onClick={() => handleVoidRequest(sale)}
                                                >
                    ...
                    {/* Void Confirmation Panel (inline, slides up) */}
                    {voidTarget && (
                        <div className="mt-6 p-6 bg-red-50 border-2 border-red-200 rounded-2xl animate-in slide-in-from-bottom-5 duration-300 space-y-5">
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 rounded-2xl bg-red-100 flex items-center justify-center shrink-0">
                                    <AlertCircle className="w-6 h-6 text-red-600" />
                                </div>
                                <div>
                                    <h4 className="text-base font-black text-red-900 uppercase tracking-tight">Konfirmasi Void Transaksi #{voidTarget.id}</h4>
                                    <p className="text-xs text-red-600/70">
                                        Total: {formatCurrency(voidTarget.totalAmount)} — Stok akan dikembalikan dan jurnal dibalik.
                                    </p>
                                </div>
                            </div>

                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-red-800/50 uppercase tracking-widest block">
                                    Masukkan Password Admin / Manager
                                </label>
                                <Input
                                    type="password"
                                    placeholder="••••••••"
                                    className="h-14 rounded-2xl border-red-200 bg-white text-center text-xl font-bold tracking-widest focus:ring-red-200 focus:border-red-400"
                                    value={adminPassword}
                                    onChange={(e) => setAdminPassword(e.target.value)}
                                    onKeyDown={(e) => e.key === "Enter" && handleVoidConfirm()}
                                    autoFocus
                                />
                                {voidError && (
                                    <p className="text-red-600 text-xs font-bold text-center animate-shake">{voidError}</p>
                                )}
                            </div>
                            ...
                            <div className="flex gap-3">
                                <Button
                                    variant="outline"
                                    className="flex-1 h-12 rounded-xl border-red-200 text-red-400 hover:bg-white font-bold"
                                ...
                                <Button
                                    className="flex-1 h-12 rounded-xl bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-wider shadow-lg shadow-red-500/20"
                                    onClick={handleVoidConfirm}
                                    disabled={voidLoading || !adminPassword}
                                >
                                    {voidLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "VOID SEKARANG"}
                                </Button>
                            </div>
                        </div>
                    )}
...
function ClosingSessionDialog({ open, onOpenChange, session, onClosing, onPrint }: any) {
    ...
    const expected = Number(session.openingBalance || 0) + Number(session.totalCashSales || 0) + Number(session.pettyCashTotal || 0);
    ...
    const handleResetAndClose = (open: boolean) => {
        if (!open) {
            setStep(1);
            setActual("");
            setNotes("");
    ...
    return (
        <Dialog open={open} onOpenChange={handleResetAndClose}>
            <DialogContent className="bg-white text-slate-900 border-slate-200 rounded-[2.5rem] max-w-md shadow-3xl p-0 overflow-hidden">
                <DialogHeader className="p-8 pb-4 border-b border-slate-100">
                    <DialogTitle className="text-2xl font-black text-blue-900 flex items-center gap-3">
                        ...
                        {step === 1 ? "Closing Kasir" : "Review Z-Report"}
                    </DialogTitle>
                </DialogHeader>
                    ...
                    {step === 1 ? (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                            <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 flex gap-4">
                                ...
                                <div>
                                    <p className="text-sm font-black text-amber-900 uppercase tracking-tight">Sistem Blind Close</p>
                                    <p className="text-[11px] text-amber-700/80 font-bold leading-relaxed mt-0.5">
                                        Hitung uang tunai di laci secara fisik. Jangan melihat angka di layar untuk menjaga integritas audit kas kantor.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Total Uang Tunai (Fisik)</label>
                                    <div className="relative">
                                        ...
                                        <Input
                                            ...
                                            placeholder="0"
                                            autoFocus
                                            className="h-20 bg-slate-50 border-slate-200 pl-16 pr-8 text-3xl font-black rounded-2xl focus:ring-blue-100 transition-all text-blue-900"
                                            value={actual}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        placeholder="Misal: Selisih Rp 500 karena pembulatan..."
                                        className="h-14 bg-white border-slate-200 rounded-xl px-6 focus:ring-blue-100 text-sm"
                                        value={notes}
                                        onChange={e => setNotes(e.target.value)}
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <Button
                                    ...
                                    onClick={onPrint}
                                >
                                    <Printer className="w-4 h-4 mr-2" /> Preview Laporan
                                </Button>
                                <Button
                                    className="h-16 rounded-2xl font-black text-xs bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-500/20 uppercase tracking-widest"
                                    onClick={handleNext}
                                    disabled={!actual}
                                >
                                    Review Selisih <ChevronRight className="w-4 h-4 ml-2" />
                                </Button>
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="space-y-3">
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">Estimasi Sistem</span>
                                    <span className="font-black text-slate-900">{formatCurrency(expected)}</span>
                                </div>
                                <div className="flex justify-between items-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                                    <span className="text-xs font-bold text-blue-600 uppercase tracking-wide">Input Fisik</span>
                                    <span className="font-black text-blue-900">{formatCurrency(actualNum)}</span>
                                </div>
                                
                                <div className={cn(
                                    "flex justify-between items-center p-5 rounded-2xl border-2 transition-all",
                                    diff === 0 ? "bg-emerald-50 border-emerald-500/30 text-emerald-700" : 
                                    diff > 0 ? "bg-blue-50 border-blue-500/30 text-blue-700" : "bg-red-50 border-red-500/30 text-red-700"
                                )}>
                                    <div className="flex items-center gap-3">
                                        <div className={cn(
                                            "w-10 h-10 rounded-full flex items-center justify-center",
                                            diff === 0 ? "bg-emerald-100" : diff > 0 ? "bg-blue-100" : "bg-red-100"
                                        )}>
                                            {diff === 0 ? <CheckCircle2 className="w-6 h-6" /> : 
                                             diff > 0 ? <TrendingUp className="w-6 h-6" /> : <TrendingDown className="w-6 h-6" />}
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Status Selisih</p>
                                            <p className="text-lg font-black uppercase tracking-tight">
                                                {diff === 0 ? "Balance Match" : diff > 0 ? "Surplus (Lebih)" : "Shortage (Kurang)"}
                                            </p>
                                        </div>
                                    </div>
                                    <span className="text-xl font-black">{formatCurrency(Math.abs(diff))}</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <Button
                                    variant="ghost"
                                    className="h-16 rounded-2xl font-black text-[10px] text-slate-400 hover:bg-slate-100 uppercase tracking-widest col-span-1"
                                    onClick={handleBack}
                                >
                                    Kembali
                                </Button>
                                <Button
                                    className="h-16 rounded-2xl font-black text-xs bg-red-600 hover:bg-red-700 text-white shadow-xl shadow-red-500/20 uppercase tracking-widest col-span-2"
                                    onClick={() => onClosing(session.id, expected, actualNum, notes)}
                                >
...
function PrinterSettingsDialog({ open, onOpenChange, paperSize, setPaperSize, onTestPrint }: any) {
    const [connectionType, setConnectionType] = useState<"usb" | "bluetooth" | "lan">(() => {
        try {
            const cfg = JSON.parse(localStorage.getItem("pos_printer_config") || "{}");
            return cfg.connectionType || "usb";
        } catch { return "usb"; }
    });
    const [bluetoothName, setBluetoothName] = useState(() => {
        try {
            return JSON.parse(localStorage.getItem("pos_printer_config") || "{}").bluetoothName || "";
        } catch { return ""; }
    });
    const [lanIp, setLanIp] = useState(() => {
        try {
            return JSON.parse(localStorage.getItem("pos_printer_config") || "{}").lanIp || "";
        } catch { return ""; }
    });
    const [lanPort, setLanPort] = useState(() => {
        try {
            return JSON.parse(localStorage.getItem("pos_printer_config") || "{}").lanPort || "9100";
        } catch { return "9100"; }
    });
    const [saved, setSaved] = useState(false);

    const handleSave = () => {
        const config = { connectionType, bluetoothName, lanIp, lanPort };
    ...
    const connectionOptions = [
        { value: "usb" as const, label: "USB / Kabel", desc: "Langsung via kabel USB", icon: Usb },
        { value: "bluetooth" as const, label: "Bluetooth", desc: "Nirkabel / Wireless", icon: Bluetooth },
        { value: "lan" as const, label: "LAN / RJ45", desc: "Jaringan Ethernet / WiFi", icon: Wifi },
    ];
            ...
            <DialogContent className="bg-white text-slate-900 border-slate-200 rounded-[2.5rem] max-w-md shadow-3xl p-0 overflow-hidden">
                ...
                <div className="p-6 pb-4 border-b border-slate-100">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-blue-900 tracking-tight">Setup Printer Thermal</h2>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Konfigurasi Perangkat Cetak</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 space-y-6 max-h-[65vh] overflow-y-auto">
                    ...
                    {/* Jenis Koneksi */}
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-500 ml-1 uppercase tracking-widest">Jenis Koneksi Printer</label>
                        <div className="grid grid-cols-3 gap-2">
                            {connectionOptions.map((opt) => {
                                const IconComp = opt.icon;
                                const isActive = connectionType === opt.value;
                                return (
                                    <button
                                        key={opt.value}
                                        onClick={() => setConnectionType(opt.value)}
                                        className={cn(
                                            "rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-1.5 p-4",
                                            isActive ? "bg-blue-50 border-blue-600 text-blue-700 shadow-md shadow-blue-500/10" : "bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100 hover:text-slate-600"
                                        )}
                                    >
                                        <IconComp className={cn("w-5 h-5", isActive ? "text-blue-600" : "text-slate-300")} />
                    ...
                    {/* Konfigurasi per Koneksi */}
                    <div className="space-y-3">
                        {connectionType === "usb" && (
                            <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100 space-y-2">
                                <div className="flex items-center gap-2 mb-1">
                                    <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                                    <span className="text-[11px] font-bold text-emerald-700 uppercase tracking-tight">USB / Kabel — Plug & Play</span>
                                </div>
                                <p className="text-[10px] text-emerald-600/80 leading-relaxed">
                                    Printer USB akan terdeteksi otomatis oleh sistem operasi Anda. Pastikan:
                                </p>
                                <ul className="text-[10px] text-emerald-600/80 space-y-0.5 ml-2">
                                    <li>✓ Kabel USB terhubung dengan baik</li>
                                    <li>✓ Driver printer sudah terinstall</li>
                                    <li>✓ Printer sudah menyala dan siap</li>
                                    <li>✓ Pilih printer yang benar saat dialog cetak muncul</li>
                                </ul>
                            </div>
                        )}
                            ...
                            <div className="space-y-3">
                                <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-100 space-y-2">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-[11px] font-bold text-indigo-700 uppercase tracking-tight">Koneksi Bluetooth</span>
                                    </div>
                                    <p className="text-[10px] text-indigo-600/80 leading-relaxed">
                                        Pastikan printer sudah di-pairing dengan perangkat ini melalui pengaturan Bluetooth sistem operasi.
                                    </p>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Nama Perangkat Bluetooth</label>
                                    <Input
                                        placeholder="Contoh: BlueTooth Printer, PT-210, RPP02N"
                                        className="h-12 bg-slate-50 border-slate-200 rounded-xl px-4 focus:ring-blue-100 text-sm"
                                        value={bluetoothName}
                                        onChange={(e) => setBluetoothName(e.target.value)}
                                    />
                                    <p className="text-[9px] text-slate-400 ml-1">Catatan: Cetak akan melalui spooler printer Bluetooth yang sudah di-pair.</p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="space-y-3">
                                <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 space-y-2">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-[11px] font-bold text-amber-700 uppercase tracking-tight">Koneksi Jaringan (LAN/RJ45/WiFi)</span>
                                    </div>
                                    <p className="text-[10px] text-amber-600/80 leading-relaxed">
                                        Masukkan alamat IP printer yang terhubung ke jaringan lokal. Port default printer thermal biasanya 9100.
                                    </p>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="col-span-2 space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Alamat IP Printer</label>
                                        <Input
                                            placeholder="192.168.1.100"
                                            className="h-12 bg-slate-50 border-slate-200 rounded-xl px-4 focus:ring-blue-100 text-sm font-mono"
                                            value={lanIp}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            placeholder="9100"
                                            className="h-12 bg-slate-50 border-slate-200 rounded-xl px-4 focus:ring-blue-100 text-sm font-mono text-center"
                                            value={lanPort}
                                            onChange={(e) => setLanPort(e.target.value.replace(/\D/g, ""))}
                                        />
                                    </div>
                                </div>
                                <p className="text-[9px] text-slate-400 ml-1">Catatan: Pastikan printer dan komputer berada di jaringan yang sama.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="p-6 pt-4 border-t border-slate-100 space-y-3">
                    <Button
                        onClick={onTestPrint}
                        variant="outline"
                        className="w-full h-13 rounded-2xl border-slate-200 text-slate-600 font-black text-[11px] uppercase tracking-widest hover:bg-slate-50"
                    >
                        <Printer className="w-4 h-4 mr-3 opacity-50" />
                        TEST PRINT ({paperSize} • {connectionOptions.find(o => o.value === connectionType)?.label})
                    </Button>
                    <Button
                        onClick={handleSave}
                        className={cn(
